...
export class TaskOdooService {
                        ...
                        for (let i = 0; i < task.photoSO.length; i++) {
                            if (task.photoSO[i]) {
                                if (knownTypes[task.photoSO[i][0]]) {
                                    task.photoSO[i] = knownTypes[task.photoSO[i][0]] + task.photoSO[i];
                                }
                            }
                        }
                    }
    ...
    requestMoreRecordTask() {
        ...
        let get_so_list = () => {
            ...
            inParams.push(['invoice_status', 'partner_id', 'date_order', 'name', 'id', 'title', 'commitment_date', 'finish']);
                                ...
                                if (line === orderLine.id) {
                                    ...
                                    switch (orderLine.product_id[0]) {
                                        ...
                                        case 40:
                                            ...
                                            if (orderLine.product_qty != orderLine.qty_invoiced) {
                    ...
                    for (let orderLine of value) {
                        for (let task of hiredListTemp) {
                            for (let line of task.facturation_line) {
                                let temp = line.findIndex((element) => element === orderLine.id);
        ...
        let list_msg_ids = () => {
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    //////console.log(value, "messages")
                    value = value.filter((messages) => {
                        return messages.subtype_id === false || messages.subtype_id[1] === 'Discussions';
                    });
                    value.reverse();
        ...
        let get_so_list = () => {
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    if (typeof applicationListTemp !== 'undefined' && applicationListTemp.length) {
                        get_po_of_task();
                    } else {
                        //!Actualizar observable
                        task$.next({ type: 0 });
        ...
        let list_msg_ids = () => {
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    value = value.filter((messages) => {
                        return messages.subtype_id === false || messages.subtype_id[1] === 'Discussions'; //&& messages.message_type !== "ranking";
                    });
                    value.reverse();
    ...
    aplicationListEdit(So_id?: number, type?: number, Task?: TaskModel, Up_Offer?: any, Up_Chat?: any) {
        switch (type) {
            case 1:
                //////////////////////////eliminar solicitud
        ...
        let get_update_so = () => {
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    if (typeof Task.Po_id_new_budget !== 'undefined' && Task.Po_id_new_budget.length > 0) {
                ...
                if (err) {
                    ...
                    if (typeof value !== 'undefined' && value.length > 0) {
                        ...
                        for (let resId of value) {
                            let temp = applicationList.findIndex((element) => element.So_id === resId.res_id);
                            if (temp !== -1) {
                                if (applicationList[temp].So_id === resId.res_id) {
                                    if (knownTypes[resId.datas[0]]) {
                                        applicationList[temp].photoSO.push(knownTypes[resId.datas[0]] + resId.datas);
                        ...
                        let temp = applicationList.findIndex((element) => element.So_id === Task.So_id);
                        if (temp !== -1) {
                            applicationList[temp].downloadPhotoSo = true;
                        }
        ...
        let get_photo_so = () => {
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    if (typeof value !== 'undefined' && value.length > 0) {
                        for (let resId of value) {
                            let temp = hiredList.findIndex((element) => element.So_id === resId.res_id);
                            if (temp !== -1) {
                                if (hiredList[temp].So_id === resId.res_id) {
                                    if (knownTypes[resId.datas[0]]) {
                                        //console.log("foto obtenida")

                                        hiredList[temp].photoSO.push(knownTypes[resId.datas[0]] + resId.datas);
            ...
            client.request('call', { service: 'object', method: 'execute_kw', args: fparams }, function (err, error, value) {
                if (err) {
                    ...
                    if (value !== 'undefined' && value.length > 0) {
                        let temp = applicationList.findIndex((element) => element.So_id === SO_id);
                        if (temp !== -1) {
                            let temp1 = applicationList[temp].So_offers.findIndex((element) => element.provider_id === provider_id);
                            if (temp1 !== -1) {
                                for (let resId of value) {
                                    ...
                                    applicationList[temp].So_offers[temp1].opinions_provider.push(coment);
                                }
                            }
                        }
                        task$.next({ type: 19 });
                        ...
                        if (temp !== -1) {
                            for (let resId of value) {
                                let temp1 = applicationList[temp].So_offers.findIndex(
                                    (element) => element.provider_id === resId.notified_partner_ids[resId.notified_partner_ids.length - 1]
                                );
                                if (temp1 !== -1) {
                                    coment = {
                                        author: resId.author_id[1],
                ...
                if (err) {
                    ...
                    if (value) {
                        ...
                        for (let poId of Po_id) {
                            let temp1 = applicationList.findIndex((element) => element.So_id === SO_id);
                            if (temp1 !== -1) {
                                let temp = applicationList[temp1].So_offers.findIndex((element) => element.Po_id === poId);
                                if (temp !== -1) {
    ...
    requestApplicationTaskAnonimus() {
        ...
        let get_po_of_task = () => {
            let inParams = [];
            inParams.push([['origin', 'in', SO_origin]]);
            inParams.push(['partner_id', 'origin', 'order_line', 'state', 'name', 'new_budget', 'amount_total']);
